...
/**
 * 		@file 	ksc-Iweb-ViewResult.php
 * 		@brief 	View query, groundtruth, and ranking result. --> Host on satoh-lab site
...
{
    printf("<P><H1>[TRECVID Instance Search Demo] - View Results </H1>\n");
...
$nTVYear = 2014;
if(isset($_REQUEST['vTVYear']))
{
    $nTVYear = $_REQUEST['vTVYear'];
}

$szTVYear = sprintf("tv%d", $nTVYear);
$szRootURL = "http://per900c.hpc.vpl.nii.ac.jp/users-ext/ledduy//www/kaori-ins2014/ksc-web-ViewResult.php";
$szParam = sprintf("vTVYear=%d", $nTVYear);

if ($nAction == 1) // Let user pick the TVYear
{
    if(isset($_REQUEST['vRunID']))
    {
        $szRunID = $_REQUEST['vRunID'];
        $szURL = sprintf("%s?%s&vRunID=%s&vAction=1", $szRootURL, $szParam, urlencode($szRunID));
    }
    else
    {
        $szURL = sprintf("%s?%s&vAction=1", $szRootURL, $szParam);       
    }
...
{
    $vPageIDPlus = intval($_REQUEST['vPageID']);
}

$nMaxVideosPerPage = 20;

if(isset($_REQUEST['vMaxVideosPerPage']))
{
    $nMaxVideosPerPage = intval($_REQUEST['vMaxVideosPerPage']);
}

$szParamTemp = $szParam;
$szParam = sprintf("%s&vShowGT=%d&vQueryID=%s&vRunID=%s&vPageID=%d&vMaxVideosPerPage=%d", $szParamTemp, $nShowGT, urlencode($szQueryIDz), urlencode($szRunID), $vPageIDPlus, $nMaxVideosPerPage);

if ($nAction == 2) // Let user pick the TVYear
{
    $szURL = sprintf("%s?%s&vAction=2", $szRootURL, $szParam);
    printf("<!--%s-->\n", $szURL);
    $szOutput1 = file_get_contents($szURL);
    
    $szOutput = str_replace("ksc-web-ViewResult.php", "ksc-Iweb-ViewResult.php", $szOutput1);
    printf("%s", $szOutput);
    exit();
...
for ($i = $nStartID; $i < $nEndID; $i ++) {
    ...
    /*
     * if(sizeof($arSelList) == 0) { print_r($arBoundingBoxList[$szShotID]); print_r($arImgList);exit(); }
     */
