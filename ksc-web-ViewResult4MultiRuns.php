...
/**
 * 		@file 	ksc-web-ViewResult4MultiRuns.php
...
// Extend to view multi-runs (max 4)
...
if ($nAction == 0) // Let user pick the TVYear
{
    ...
    printf("<P>Select Max Runs to View<BR>\n");
...
$arRunID = $_REQUEST['vRunID'];
$nNumRuns = sizeof($arRunID);
...
//    Number of discarded images: 1

    $arTmp1 = explode(":", $arRawListz[1]);
    // print_r($arTmp1);
    $nNumTrainingImages = intval($arTmp1[1]);

    $arTmp1 = explode(":", $arRawListz[2]);
    // print_r($arTmp1);
    $nNumDiscardImages = intval($arTmp1[1]);
...
$arOutput[] = sprintf("<P><H1>Total number of training images: [%d] - Number of discarded images: [%s]</H1><BR>\n", $nNumTrainingImages, $nNumDiscardImages);
...
foreach ($arRunID as $szRunID) {
    ...
    $arAllRawList[$szRunID] = $arRawList;
    
    $nNumVideos = min($nNumVideos, sizeof($arRawList));
}
...
$szRunIDParam = "";
foreach ($arRunID as $szRunIDz) {
	$szRunIDParam = $szRunIDParam . urlencode("vRunID[]") . "=" . urlencode($szRunIDz) . "&";
}

$queryURL = sprintf("%svQueryID=%s&vMaxVideosPerPage=%s&vTVYear=%d&vAction=%d&", 
    $szRunIDParam, urlencode($szQueryIDz), urlencode($nMaxVideosPerPage), $nTVYear, $nAction);
// printf($queryURL);


$arPageOutput = array();
$arPageOutput[] = sprintf("<P><H2>Page: ");
for ($i = 0; $i < $nNumPages; $i ++) {
    if ($i != $nPageID) {
        $szURL = sprintf("ksc-web-ViewResult4MultiRuns.php?%s&vPageID=%d&vShowGT=%d", $queryURL, $i + 1, $nShowGT);
        $arPageOutput[] = sprintf("<A HREF='%s'>%02d</A> ", $szURL, $i + 1);
    } else {
        $arPageOutput[] = sprintf("%02d ", $i + 1);
    }
}

//print_r(array_keys($arAllRawList));exit();
foreach ($arAllRawList as $szRunID => $arRawList) {
    $arScoreList = array();
    ...
    $arRunOutput[$szRunID][] = sprintf("<P><H3>MAP: %0.2f. Num hits (@1000): %d.<BR>\n", $fMAP, $nTotalHitsz);
    // //
    ...
    $queryURLz = sprintf("vRunID=%s&vQueryID=%s&vMaxVideosPerPage=%s&vTVYear=%d&vAction=%d&", urlencode($szRunID), urlencode($szQueryIDz), urlencode($nMaxVideosPerPage), $nTVYear, $nAction);
    
    $szURLz = sprintf("ksc-web-ViewResult.php?%s&vShowGT=1", $queryURLz);
    
    $nViewImg = 0;
    
    $arRunOutput[$szRunID][] = sprintf("<P><H3>Total Relevant Videos <A HREF='%s'>[%s]</A>.</H3>\n", $szURLz, sizeof($arNISTList[$szQueryID]));
    
    $arRunOutput[$szRunID][] = sprintf("<BR>\n");
    $nHits = 0;
    ...
    for ($i = $nStartID; $i < $nEndID; $i ++) {
        ...
        $arRunOutput[$szRunID][] = sprintf("%d. ", $nCount + 1);
        $nCountz = 0;
        $nSampling = 0;
        ...
        foreach ($arImgList as $szImg) {
            ...
            $szURLImg = sprintf("%s/%s/%s/%s", $szKeyFrameDir, $szPatName4KFDir, $szShotID, $szImg);
            // /
            ...
            $arRunOutput[$szRunID][] = sprintf("<A HREF='%s' TARGET=_blank><IMG  TITLE='%s - %s' SRC='data:image/jpeg;base64," . $szImgContent . "' /></A>", $szURL, $szShotID, $fScore);
...
/*        
        $arRunOutput[$szRunID][] = sprintf("[%s-%0.6f]\n", $szShotID, $fScore);
        if ($szPrevData != "") {
            $arRunOutput[$szRunID][] = sprintf("[%s]\n", $szPrevData);
        }
*/        
        if (in_array($szShotID, $arNISTList[$szQueryID])) {
            $arRunOutput[$szRunID][] = sprintf("<IMG SRC='winky-icon.png'><BR>\n");
            $nHits ++;
        } else {
            if (in_array($szShotID, $arJudgedShots[$szQueryID])) {
                $arRunOutput[$szRunID][] = sprintf("<IMG SRC='sad-icon2.png'><BR>\n");
            } else {
                $arRunOutput[$szRunID][] = sprintf("<IMG SRC='unknown-icon.png' WIDTH=50><BR>\n");
            }
        }
    ...
    $arRunOutput[$szRunID][] = sprintf("<P><H3>Num hits (top %s): %d/%d.</H3>\n", $nMaxVideosPerPage, $nHits, $nTotalHits);
}
...
printf("<P><H1>Run List</H1>\n");
$arKeys = array_keys($arRunOutput);
$nCountzz = 1;
foreach($arKeys as $szRunID)
{
    $arRunShortCut[$szRunID] = sprintf("[Run %d]", $nCountzz);
    printf("<H3>%s: %s</H3>\n", $arRunShortCut[$szRunID], $szRunID);
    $nCountzz++;
}
$nMaxRuns = sizeof($arRunOutput);
$nSize = intval(100/$nMaxRuns);
//printf("Size: %d/%d", $nSize, $nMaxRuns);
printf("<TABLE BORDER=\"1\" WIDTH=\"100%%\">\n");
printf("<TR>\n");
foreach ($arRunOutput as $szRunID => $arLine) {
    printf("<TD WIDTH=\"%d%%\"><H2>%s</H2></TD>\n", $nSize, $arRunShortCut[$szRunID]);
}
printf("</TR>\n");
...
foreach ($arPageOutput as $szLine) {
    printf("%s\n", $szLine);
}
// ob_flush_end();
exit();
