...
files = dir(fullfile(database.bow_dir, [file_name '*']));
...
clip_id = 0;
for i=1:nfile
	fprintf('\r(%d~%d)', i, nfile);
	load(fullfile(database.bow_dir,files(i).name));
	num_new_shot = size(sub_list_frame_bow,2);
	if save_raw_bow
		list_frame_bow(clip_id+1:clip_id+num_new_shot) = sub_list_frame_bow;
	end
	if save_max_avg_bow
		list_avg_pooling_bow(:,clip_id+1:clip_id+num_new_shot) = cell2mat(cellfun(@(x) mean(x,2), sub_list_frame_bow,'UniformOutput', false));
		%mean(sub_list_frame_bow{j},2);
	end
	list_clip_frame_num(clip_id+1:clip_id+num_new_shot) = cell2mat(cellfun(@(x) size(x,2), sub_list_frame_bow,'UniformOutput', false));
	list_term_freq.occu  = list_term_freq.occu  + sub_list_term_freq.occu;
	list_term_freq.clip  = list_term_freq.clip  + sub_list_term_freq.clip;
	list_term_freq.frame = list_term_freq.frame + sub_list_term_freq.frame;
	
	clip_id = clip_id + num_new_shot;
	clear sub_list_term_freq; clear sub_list_frame_bow;
end
...
if save_raw_bow
	fprintf('\n save big bow');
	save(raw_bow_file, 'list_term_freq','list_frame_bow', 'list_id2clip_lut', '-v7.3');
end
...
if save_max_avg_bow && idf_l1_norm 
