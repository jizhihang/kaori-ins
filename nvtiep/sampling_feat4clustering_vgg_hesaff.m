...
clobber = false;
list_mat_file = ['/net/per610a/export/das11f/ledduy/plsang/nvtiep/INS/INS2013/meta/', feature_name, '_mat_dir.mat'];
...
nfeat_files = dir(['/net/per610a/export/das11f/ledduy/plsang/nvtiep/INS/INS2013/meta/',feature_name,'_*']);
if length(nfeat_files) ~= length(range) % check da co du thong tin so luong feat chua
	...
	fid = fopen(['/net/per610a/export/das11f/ledduy/plsang/nvtiep/INS/INS2013/meta/',feature_name,'_',num2str(range(rid)),'_',num2str(database.totalfeatures), '_feats.txt'], 'w');
	fclose(fid);
