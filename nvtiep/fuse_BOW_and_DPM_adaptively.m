function fuse_BOW_and_DPM_adaptively
% Example: 
% fuse_BOW_and_DPM_adaptively(1, 2)
...
base_BOW = 'R2_tv2013.surrey.hard.soft.latefusion.asym_fg+bg_0.1_hesaff_rootsift_noangle_akmeans_1000000_100000000_50_kdtree_8_800_v1_f1_1_avg_pooling_full_notrim_clip_idf_nonorm_kdtree_3_0.0125_-1_dist_avg_autoasym_ivf_0.5';
...
base_BOW_fusion = 'R1_tv2013.fusion.adaptive-surrey.hard.soft+DPM';
base_RANSAC = 'R4_rawRANSAC_tv2013.surrey.hard.soft.latefusion.asym'; % new
...
if isempty(strfind(base_BOW, base_feature))
	disp ('Insconsistency between base_featue and base_RANSAC');
	quit;
end
...
RESULT_RUN_ID = ['R0_', data_name, '.', base_BOW_fusion, '+DPM+woRANSAC_old'];

if is_using_RANSAC
	RESULT_RUN_ID = ['R0_', data_name, '.', base_BOW_fusion, '+DPM+RANSAC_newformula22'];
end
			...
			for shot_idx = 1:nshot	% duyet qua tat ca cac shot trong danh sach cua RANSAC
				...
				for frame_idx=1:length(frame_locs) % duyet qua tat ca cac frame ma co su dung DPM
					...
					if ~isempty(inliers_struct.fg_inlier_loc{shot_idx}{loc})
						...
						%fg_kp = unique(fg_kp', 'rows')'; % bo cai nay thanh formula 8, formula 9
					end
					if ~isempty(inliers_struct.bg_inlier_loc{shot_idx}{loc})
						...
						%bg_kp = unique(bg_kp', 'rows')'; % bo cai nay thanh formula 8, formula 9
						%bg_kp = unique(bg_kp', 'rows')'; % de cai nay thanh formular 23
					end
					...
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula23 
					new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*sqrt(max(1,N_bg))*P_score; 	% formula22
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula21
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*sqrt(max(1,Nd))*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula20
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula19 
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*log(max(2,N_bg))*P_score; 	% formula18  log nepe
					%new_scores(end+1) = max(1,Nd)*max(1,N_fg-Nd)*sqrt(max(1,N_fg-Nd))*log2(max(2,N_bg))*P_score; 	% formula17
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*sqrt(max(1,N_fg-Nd))*log2(max(2,N_bg))*P_score; 	% formula16
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*max(1,N_fg-Nd)*max(1,N_bg)*P_score; 	% formula15 
					%new_scores(end+1) = max(1,Nd)*sqrt(max(1,Nd))*max(1,N_fg-Nd)*sqrt(max(1,N_fg-Nd))*log2(max(2,N_bg))*P_score; 	% formula14
					%new_scores(end+1) = max(1,Nd)*sqrt(max(1,Nd))*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula13
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*sqrt(max(1,N_fg-Nd))*log2(max(2,N_bg))*P_score; 	% formula12
					%new_scores(end+1) = max(1,Nd)*sqrt(max(1,N_fg-Nd))*log2(max(2,N_bg))*P_score; 	% formula11
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula10
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula9
					%new_scores(end+1) = max(1,Nd)*max(1,Nd)*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula8
					%new_scores(end+1) = max(1,Nd)*max(1,N_fg-Nd)*log2(max(2,N_bg))*P_score; 	% formula7
					%new_scores(end+1) = max(1,N_fg)*max(1,N_fg)*log2(max(2,N_bg))*P_score; 	% formula6
				...
				if ~debug_mode
					fprintf(fid, '%s #$# %s #$# %f\n', shot, shot, max(new_scores)); % formula21: (max+mean)/2
				end
			end
			if ~debug_mode
				fclose(fid);
	...
	for q_id = start_query_id:end_query_id	% Duyet qua tat ca cac cau query
		...
		for id = start_video_id:end_video_id
			...
			for shot_idx = 1:nshot	% duyet qua tat ca cac shot trong danh sach cua RANSAC
				...
				load(db_quant_file, 'bins');		% dung de lay thong tin bins
				db_frame_info_file = fullfile(db_frame_info_dir, [shot,'.mat']);
				load(db_frame_info_file, 'clip_frame', 'clip_kp');	% dung de lay thong tin clip_frame
				...
				for frame_idx=1:length(frame_locs) % duyet qua tat ca cac frame ma co su dung DPM
					re = '.*_KSC(.*)';
					[rematch, frame_name] = regexp(frame_names{frame_locs(frame_idx)}, re, 'match', 'tokens');
					[isa, loc] = ismember(frame_name{end}{1}, clip_frame);
