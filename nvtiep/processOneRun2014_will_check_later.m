...
if exist(res_filename, 'file')
	...
	if database.comp_sim.query_num ~= 1 && isempty(strfind(database.comp_sim.query_agg,'max_pooling'))...
		...
		for qid = 1:query_num
			...
			for sid = 1:subset_num
				...
				if ~isempty(strfind(database.comp_sim.query_agg,'avg'))
					ntopics = size(dist, 2)
					new_dist = cell(1,ntopics);
					for tt = 1:ntopics
						new_dist{tt} = mean(dist(1:tt),2);
					end
					%dist = mean(dist,2);
				end
				%% Tiep: New code here, please check first if buggy!!!
				dists{qid}{sid} = new_dist;
			end
			fprintf(' %.0f',toc);
		end
		fprintf('\n');
	...
	tic;
	fprintf('sorting ...');
	ntopics = size(dists{1}{1}, 2);
	
	%score = cell(1,query_num);
	%ranks = cell(1,query_num);
	score = cell(1,ntopics);
	ranks = cell(1,ntopics);
	for tt=1:ntopics
		score{tt} = cell(1,query_num);
		ranks{tt} = cell(1,query_num);
		for qid = 1:query_num
			subset_num = length(topic_bows{qid});
			score{tt}{qid} = cell(1,subset_num);
			ranks{tt}{qid} = cell(1,subset_num);
			for sid = 1:subset_num
				[score{tt}{qid}{sid},ranks{tt}{qid}{sid}]=sort(dists{qid}{sid}{tt},1);
			end
		end
	end
	fprintf(' %.0f\n',toc);
...
for tt = 1:length(score)
	knn_txt_dir = fullfile(result_dir, ['txt' num2str(tt)]); % result/tv2014/test2014/runID/txt (runID = res_name)
	if ~exist(knn_txt_dir,'dir')
		mkdir(knn_txt_dir);
		fileattrib(knn_txt_dir,'+w','a');
	end

	write_knn(query_filenames, db_lut, score{tt}, ranksscore{tt}, database.db_frame_dir, ...
