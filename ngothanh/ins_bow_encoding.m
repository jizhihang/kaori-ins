...
for i=max(1,istart):min(iend,length(prg))
    ...
    % file to export
    ofilename = [cfg.localdir '/' char(curprg) '.feature-ext'];    
    % for each  shot
    for j=1:length(u)        
		...
		totalshot = totalshot + 1;
            ...
            if (~isempty(frames))       
                ...
                fprintf(fo,'%s %d',char(u(j)),length(shotBow));
...
toc;
disp(['Total shot = ' num2str(totalshot)]);
disp(['Total frame = ' num2str(totalframe)]);
disp('--DONE--');