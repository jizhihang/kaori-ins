function ins13SampleSIFT(rawfeatdir,untardir,datadir,year,feattype,nsample,param1,param2,param3)
%% scripts to sampling extracted SIFT(s)
cfg.rawfeatdir = rawfeatdir;
cfg.untardir = untardir;
cfg.datadir = datadir;
cfg.year = year;    % = 'tv2011' / 'tv2012' / 'tv2013'
cfg.type = 'test';      % = 'test' / 'query'
cfg.feattype = feattype; % = 'heslap'
cfg.nsample = nsample;
...
for i=1:nseltar
    ...
    nselfeat = floor(param2*length(utarfile));
    ...
    for j=1:nselfeat
        ...
        feats = feats(:,randperm(floor(param3*size(feats,2))));
