...
/*
 STEP 1: Load tar data and untar
 STEP 2: Convert raw data 2 dvf format, save with name DataName-c0-b0.dvf
 STEP 3: Perform sashKeyPointTool -findNN
 --> output file is write in a+t mode
 --> szAnn must have all info, sashKeyPointTool only add number of keypoints
 */
...
$szTmpDir = sprintf("%s/%s/bow.codebook.%s.%s/%s/%s-%d-%d", $szLocalTmpDir, $szScriptBaseName,
		$szTrialName, $szPatName, $szFeatureExtInput, $szTargetPatName, $nStartID, $nEndID); // Update Aug 15, 2013
makeDir($szTmpDir);

// !!! IMPORTANT
// source sash data
$szRootCentroidDir1 = sprintf("%s/bow.codebook.%s.%s/%s",
		$szRootFeatureDir, $szTrialName, $szPatName, $szFeatureExtInput);
...
$szSashCentroidName =  sprintf("%s.%s.%s.Centroids",
		$szTrialName, $szPatName, $szFeatureExtInput);
...
$szFeatureExtOutput = sprintf("%s.%s.%s",
		str_replace("raw", "bow", $szFeatureExtInput), $szTrialName, $szPatName);
...
computeBoWFeatureForOnePat($szTmpDir,
		$szRootFeatureOutputDir, $szRootFeatureInputDir,
		$szSashCentroidDir, $szSashCentroidName,
		$szRootMetaDataDir, $szFPVideoListFN,
		$szFeatureExtOutput, $szFeatureExtInput, $nSamplingInterval,
		$szPrefixAnn, $nStartID, $nEndID);
...
function computeBoWFeatureForOneVideoProgram($szLocalDir,
		$szFPKeyFrameListFN, $szSashCentroidDir, $szSashCentroidName,
		$szRootFeatureOutputDir, $szRootFeatureInputDir, $szVideoPath, $szVideoID,
		$szFeatureExtInput, $szFeatureExtOutput, $szPrefixAnn, $nSamplingInterval)
{
...
// One video program consists of a list of keyframe images
function computeBoWFeatureForOnePat($szLocalDir,
		$szRootFeatureOutputDir, $szRootFeatureInputDir,
		$szSashCentroidDir, $szSashCentroidName,
		$szRootMetaDataDir, $szFPVideoListFN,
		$szFeatureExtOutput, $szFeatureExtInput, $nSamplingInterval=10,
		$szPrefixAnn="", $nStartID=-1, $nEndID=-1)
{
	...
	for($i=$nStartID; $i<$nEndID; $i++)
	{
		...
		computeBoWFeatureForOneVideoProgram($szLocalDir,
				$szFPKeyFrameListFN, $szSashCentroidDir, $szSashCentroidName,
				$szRootFeatureOutputDir, $szRootFeatureInputDir, $szVideoPath, $szVideoID,
				$szFeatureExtInput, $szFeatureExtOutput, $szPrefixAnn, $nSamplingInterval);
	}
