...
$szPatName = "test2013-new"; 
$szVideoPath = "tv2013/test2013-new"; // path from archive name to keyframe dir, eg. tv2007/devel
$nStartVideoID = 0;
$nEndVideoID = 1;
$nStartKFID = 0;
$nEndKFID = 100000;
...
if($argc != 9)
{
	...
	printf("Usage: %s %s %s %s %s %s %s %s %s\n", $argv[0], $szFeatureExt, $szFeatureConfigParam, $szPatName, $szVideoPath, 
			$nStartVideoID, $nEndVideoID, $nStartKFID, $nEndKFID);
	exit(1);
}
...
$arLog[] = sprintf("###Start [%s --> $$$]: [%s]-[%s]-[%s]-[%s]-[%s]-[%s]-[%s]-[%s]",
$szStartTime,
$argv[1], $argv[2], $argv[3], $argv[4], $argv[5], $argv[6], $argv[7], $argv[8]);
saveDataFromMem2File($arLog, $szFPLogFN, "a+t");

extractRawSIFTFeature($szRootFeatureDir, $szRootKeyFrameDir,
$szRootMetaDataDir, $szRootPrgFileDir, $szPatName, $szVideoPath,
$szFeatureExt, $szFeatureConfigParam,
$nStartVideoID, $nEndVideoID, $nStartKFID, $nEndKFID);

$arLog = array();
$szFinishTime = date("m.d.Y - H:i:s");
$arLog[] = sprintf("###Finish [%s --> %s]: [%s]-[%s]-[%s]-[%s]-[%s]-[%s]-[%s]-[%s] ",
 ...
 *
 * @param $szAppName
 * @param $szFPFeatureConfigFN
 * @param $szFPInputImgFN
 * @param $szFPFeatureOutputFN
 * @param $szTmpDir
 */
function extractRawSIFTFeatureForOneImage($szAppName, $szFeatureConfigParam,
$szFPInputImgFN, $szFPFeatureOutputFN, $szTmpDir="/tmp")
{
	...
	if($gSkippExistingFiles)
	{
		...
		if(
				file_exists($szFPTarGzFeatureOutputFN) 
				&& file_exists($szFPTarGzSimpleFeatureOutputFN)
				&& filesize($szFPTarGzFeatureOutputFN)
				&& filesize($szFPTarGzSimpleFeatureOutputFN))
		{
			$nSkipByOutOfDate = 1;
	...
	if(file_exists($szFPFeatureTmpFN) && file_exists($szFPSimpleFeatureTmpFN))
	{
		...
		$szFPTarFeatureTmpFN, $szTmpDir, $szBaseOutputName);
		execSysCmd($szCmdLine);
		$arCmdLine[] =  $szCmdLine;
		...
		// if using -C <DirName>, the remaing one is only file name (no path)
		$szCmdLine = sprintf("tar -cvzf %s -C %s %s.loc",
		$szFPTarSimpleFeatureTmpFN, $szTmpDir, $szBaseOutputName);
		execSysCmd($szCmdLine);
		$arCmdLine[] =  $szCmdLine;
...
function extractRawSIFTFeature($szRootFeatureDir, $szRootKeyFrameDir,
$szRootPatFileDir, $szRootPrgFileDir,
$szPatName, $szVideoPath,
$szFeatureExt, $szFeatureConfigParam, 
$nStartVideoID, $nEndVideoID, $nStartKFID, $nEndKFID)
{
	...
	for($iz=$nStartVideoID; $iz<$nEndVideoID; $iz++)
	{
		...
		for($jkf=$nLocalStartKFID; $jkf<$nLocalEndKFID; $jkf++)
		{
			...
			$szFPInputImgFN = sprintf("%s/%s.%s",
					$szLocalKeyFrameDir2, $szKeyFrameID, $gKeyFrameImgExt);
			
			$szFPFeatureOutputFN = sprintf("%s/%s.%s", $szFeatureOutputDir, $szKeyFrameID, $szFeatureExt);
			
			//--> Modified Jul 06, 2012 -- $szLocalTmpFeatureDir
			extractRawSIFTFeatureForOneImage($szAppName, $szFeatureConfigParam,
			$szFPInputImgFN, $szFPFeatureOutputFN, $szLocalTmpFeatureDir2); 	
		}
